...
class _DbarItem:
    ...
    def finalize(self, value_format, color_indices, group_indices):
        ...
        self.color_width = self.group_draw_width / (len(color_indices) + 1)
...
def _plot_dbar(ax, items: List[_DbarItem], group_label, value_label):
...
def grbar_plot(
         ...
         color_column: Union[str, pd.Series],
    ...
    return _plot_dbar(default_ax(ax), items, group_label, value_label)
